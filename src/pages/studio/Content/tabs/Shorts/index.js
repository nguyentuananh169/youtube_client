...
function Shorts({ tab }) {
    ...
    useEffect(() => {
        if (tab === 'shorts' && isLoadingFirst) {
            getVideoApi();
        }
    }, [tab, isLoadingFirst]);
    const getVideoApi = async (valuePage, valueLimit) => {
        setIsLoading(true);
        const obj = {
    ...
    const handleClickBtnUpdate = (item) => {
        ...
        navigate('/studio/videos/shorts?type=update_video');
    };
            ...
            <div className={clsx(styles.body)}>
                ...
                {!videoList.length && !isLoading && (
                    <NoData
                        ...
                        isBottomBtn
                        bottomLink="/studio/videos/shorts?type=upload_video"
                    />
                )}
            </div>
        </>
