...
function Shorts() {
    ...
    useEffect(() => {
        if (id && page === 'shorts' && isLoadingFirst) {
            fetchVideoList();
        }
    ...
    useEffect(() => {
        if (page === 'shorts' && hasMore && !isLoading && params.page <= params.totalPage) {
            fetchVideoList();
        }
    }, [hasMore]);
    useEffect(() => {
                    ...
                    {!isLoadingFirst &&
                        videoList.map((item) => (
                            <div key={item.video_id} className={clsx(styles.videoCard)}>
                                <VideoCard hidenOwner item={item} isAspectRatioShort />
                            </div>
                        ))}
                    {isLoading &&
                        isLoadingFirst &&
                        ...
                        Array(4)
                            ...
                            .map((item, index) => (
                                <div key={index} className={clsx(styles.videoCard)}>
                                    <VideoCardLoading key={index} hidenOwner isAspectRatioShort />
                                </div>
                            ))}
